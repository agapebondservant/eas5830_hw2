		...
		if block.transactions:
			for tx in block.transactions:
				if tx.get('maxFeePerGas') is not None and tx.get('maxPriorityFeePerGas') is not None:
					...
					elif block.get('baseFeePerGas') is not None:
...
def get_contract_values(contract, admin_address, owner_address):
	...
	onchain_root = 0 #contract.getMerkleRoot()  # Get and return the merkleRoot from the provided contract
	has_role = 0 #contract.hasRole(default_admin_role, admin_address)  # Check the contract to see if the address "admin_address" has the role "default_admin_role"
	prime = contract.getPrimeByOwner(owner_address)  # Call the contract to get the prime owned by "owner_address"
